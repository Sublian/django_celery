    ...
    try:
        ...
        try:
            ...
            partners_con_ruc = Partner.objects.filter(
                document_type="ruc", num_document__isnull=False
            ...
            for partner in partners_con_ruc:
                ...
                try:
                    ...
                    if resultado.get("success"):
                        ...
                        print(
                            f"   📛 Razón Social API: {resultado.get('nombre_o_razon_social')}"
                        )

                        # Comparar con lo que tenemos
                        if partner.name != resultado.get("nombre_o_razon_social"):
                            print(
                                f"   ⚠️  Nombre difiere: BD='{partner.name}' vs API='{resultado.get('nombre_o_razon_social')}'"
                            )

                        # Validar para facturación
                        validacion = client.validar_ruc_para_facturacion(
                            partner.num_document
                        )
            ...
            partners_con_dni = Partner.objects.filter(
                document_type="dni", num_document__isnull=False
                ...
                try:
                    ...
                    if resultado.get("success"):
                        ...
                        if partner.name != resultado.get("nombre"):
                            print(
                                f"   ⚠️  Nombre difiere: BD='{partner.name}' vs API='{resultado.get('nombre')}'"
                            )
            ...
            datos_prueba = [
                {
                    "document_type": "ruc",
                    "num_document": "20603274742",
                    "name": "MIGO S.A.C.",
                },
                {
                    "document_type": "dni",
                    "num_document": "71265310",
                    "name": "INCA SIERRA YENNY MELISSA",
                },
                {
                    "document_type": "ruc",
                    "num_document": "20131312955",
                    "name": "EJEMPLO INACTIVO",
                },
            ]
            ...
            for dato in datos_prueba:
                ...
                try:
                    if dato["document_type"] == "ruc":
                        ...
                        if resultado.get("success"):
                            print(
                                f"   ✅ RUC válido: {resultado.get('nombre_o_razon_social')}"
                            )
                        else:
...
def test_particion_rucs():
    ...
    rucs_prueba = [
        f"{i:011d}" for i in range(1, 251)
    ]  # RUCs del 00000000001 al 00000000250
    ...
    print(
        f"   📊 Distribución: {' + '.join(str(len(l)) for l in lotes_100)} = {sum(len(l) for l in lotes_100)}"
    )
                ...
                if endpoint.name == "consulta_dni":
                    ...
                    try:
                        ...
                        if (
                            ...
                            and resultado.get("success") == True
                        ):
                            print(
                                f"   ✅ Consulta masiva exitosa: {resultado.get('successful', 0)} de {resultado.get('total_requested', 0)} RUCs procesados"
                            )
                            print(
                                f"   📊 Activos: {resultado['summary']['activos']}, Habidos: {resultado['summary']['habidos']}"
                            )
                            ...
                            if analisis:
                                ...
                                print(
                                    f"   📈 Habilitados para facturar: {analisis['habilitados_facturacion']['cantidad']} ({analisis['habilitados_facturacion']['porcentaje']:.1f}%)"
                                )
                                print(
                                    f"   ⚠️  No habilitados: {analisis['no_habilitados_facturacion']['cantidad']} ({analisis['no_habilitados_facturacion']['porcentaje']:.1f}%)"
                                )
                                ...
                                if analisis["advertencias"]["items"]:
                                    ...
                                    for item in analisis["advertencias"]["items"][
                                        :2
                                    ]:  # Mostrar primeras 2
                                        print(
                                            f"      • {item['ruc']}: {', '.join(item['advertencias'])}"
                                        )
                            ...
                            filename = (
                                f"{results_dir}/consulta_ruc_masivo_{timestamp}.json"
                            )
                            ...
                            try:
                                ...
                                # Probar partición
                                lotes = client._particionar_rucs_en_lotes(
                                    rucs_largos, 30
                                )  # Lotes de 30
                                print(
                                    f"   📦 Lista de {len(rucs_largos)} RUCs particionada en {len(lotes)} lotes"
                                )
                                print(
                                    f"   📊 Distribución por lote: {[len(l) for l in lotes]}"
                                )

                                # Guardar también la estructura de partición
                                particion_file = (
                                    f"{results_dir}/particion_rucs_{timestamp}.json"
                                )
                        ...
                        if "Rate limit" in str(e):
                            print(
                                f"   ⚠️  Rate limit alcanzado. Prueba aumentando time.sleep()"
                            )
                        ...
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        error_file = (
                            f"{results_dir}/error_consulta_masiva_{timestamp}.json"
                        )
                    ...
                    fecha_ayer = (timezone.now() - timedelta(days=1)).strftime(
                        "%Y-%m-%d"
                    )
                    resultado = client.consultar_tipo_cambio_fecha(fecha_ayer)
                    if resultado.get("success"):
                        print(
                            f"   ✅ Tipo cambio {fecha_ayer}: {resultado.get('precio_compra')}"
                        )
                        endpoints_exitosos += 1
                    ...
                    if resultado.get("success"):
                        print(
                            f"   ✅ Representantes encontrados: {len(resultado.get('data', []))}"
                        )
                        endpoints_exitosos += 1

                elif endpoint.name == "tipo_cambio_rango":
                    fecha_inicio = (timezone.now() - timedelta(days=7)).strftime(
                        "%Y-%m-%d"
                    )
                    fecha_fin = timezone.now().strftime("%Y-%m-%d")
                    resultado = client.consultar_tipo_cambio_rango(
                        fecha_inicio, fecha_fin
                    )
                    if isinstance(resultado, list) and len(resultado) > 0:
                        print(
                            f"   ✅ Tipo cambio rango exitoso: {len(resultado)} registros"
                        )
                        endpoints_exitosos += 1
...
def test_tipo_cambio_uso_real():
    ...
    try:
        ...
        if resultado_fecha.get("success"):
            ...
            dif_compra = float(resultado_latest.get("precio_compra", 0)) - float(
                resultado_fecha.get("precio_compra", 0)
            )
            dif_venta = float(resultado_latest.get("precio_venta", 0)) - float(
                resultado_fecha.get("precio_venta", 0)
            )

            print(
                f"   📈 Variación vs hoy: Compra: {dif_compra:+.4f}, Venta: {dif_venta:+.4f}"
            )
        else:
...
def test_rate_limit_endpoint_especifico():
    ...
    try:
        ...
        endpoints_personalizados = service.endpoints.filter(
            is_active=True, custom_rate_limit__isnull=False
        )
        ...
        if endpoints_personalizados.exists():
            for endpoint in endpoints_personalizados:
                ...
                print(
                    f"     2. Lógica en _check_rate_limit que use endpoint.custom_rate_limit"
                )

        else:
            print("   ℹ️  No hay endpoints con límites personalizados")
            print(
                "   💡 Sugerencia: Configura custom_rate_limit en endpoints DNI y RUC masivo"
            )
...
def test_estadisticas_uso():
    ...
    try:
        ...
        logs_24h = ApiCallLog.objects.filter(
            service=service, created_at__gte=ultimas_24h
        )
        logs_semana = ApiCallLog.objects.filter(
            service=service, created_at__gte=ultima_semana
        )
        ...
        for estado in estados:
            porcentaje = (
                (estado["total"] / logs_24h.count() * 100)
                if logs_24h.count() > 0
                else 0
            )
        ...
        endpoints = (
            logs_24h.values("endpoint__name")
            .annotate(
        ...
        if promedio_hora > service.requests_per_minute:
            print(
                f"   ⚠️  Uso promedio por hora ({promedio_hora:.1f}) cerca del límite/min"
            )
        else:
            print(
                f"   ✅ Uso promedio por hora ({promedio_hora:.1f}) dentro de límites"
            )

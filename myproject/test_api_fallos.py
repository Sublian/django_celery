...
def test_token_invalido():
    ...
    try:
        ...
        logs = ApiCallLog.objects.filter(error_message__icontains="token").order_by(
            "-created_at"
        )[:1]
...
def test_rate_limiting():
    ...
    try:
        ...
        if errores_rate_limit > 0:
            print(
                f"   ✅ Correcto: Se detectaron {errores_rate_limit} errores de rate limit"
            )
        else:
            print(
                "   ⚠️  No se detectaron errores de rate limit (puede que el límite sea alto)"
            )
...
def test_rate_limiting_intensivo():
    ...
    try:
        ...
        if rate_limited > 0:
            ...
            print(
                f"  ⚠️ No se activó rate limiting (límite: {service.requests_per_minute}/min)"
            )
...
def test_rate_limiting_dni_rapido():
    ...
    try:
        ...
        endpoint_dni = ApiEndpoint.objects.filter(
            service=service, path="/api/v1/dni"
        ).first()
        ...
        try:
            ...
            if logs:
                ...
                print(f"   📝 Log creado: ID={log.id}")
                print(
                    f"   🕒 Hace: {(timezone.now() - log.created_at).seconds} segundos"
                )
                return True
            else:
...
def test_rate_limiting_logs_completos():
    ...
    try:
        ...
        print(
            f"1. Rate limit forzado: {rate_limit_obj.current_count}/{service.requests_per_minute}"
        )
        ...
        for metodo, nombre_endpoint in endpoints_a_probar:
            ...
            try:
                ...
                if logs:
                    ...
                    print(
                        f"   🔧 Endpoint en log: {log.endpoint.name if log.endpoint else 'N/A'}"
                    )
                    ...
                    if "attempted_endpoint" in log.request_data:
                        ...
                        if endpoint_intentado.lower() in nombre_endpoint.lower():
                            ...
                            print(
                                f"   ⚠️  Endpoint en log NO coincide: {endpoint_intentado} vs {nombre_endpoint}"
                            )
                    else:
        ...
        if logs_creados:
            ...
            for i, log in enumerate(logs_creados, 1):
                ...
                print(f"  ID: {log.id}")
                print(
                    f"  Endpoint DB: {log.endpoint.name if log.endpoint else 'No asignado'}"
                )
                print(
                    f"  Endpoint intentado: {log.request_data.get('attempted_endpoint', 'No registrado')}"
                )
...
def test_validacion_ruc_inactivo():
    ...
    try:
        ...
        print(
            "Nota: Para probar RUC inactivo, necesitamos un RUC real que esté INACTIVO"
        )
        ...
        ruc_test = input(
            "\nIngresa un RUC para probar (o presiona Enter para omitir): "
        ).strip()
        ...
        if logs:
            ...
            for log in logs:
                print(
                    f"      - {log.endpoint.name if log.endpoint else 'N/A'}: {log.status}"
                )
...
def verificar_trazabilidad_completa():
    ...
    endpoints_populares = (
        logs.values("endpoint__name").annotate(total=Count("id")).order_by("-total")[:5]
    )

...
async def test_single_request_with_logging():
    ...
    service_exists = await sync_to_async(ApiService.objects.filter(name="NubeFact").exists)()
    ...
    try:
        ...
        if final_count > initial_count:
            ...
            if latest:
                ...
                print(f"   Endpoint: {latest.endpoint.name if latest.endpoint else 'N/A'}")
            ...
            for log in logs:
                print(f"   - {log.created_at}: {log.endpoint.name if log.endpoint else 'N/A'} ({log.status})")

...
class InvoiceTestRunner:
    ...
    def create_test_invoice_data(self, invoice_number=None):
        ...
        return {
            ...
            "fecha_de_vencimiento": (datetime.now() + timedelta(days=2)).strftime(
                "%Y-%m-%d"
            ),
            "moneda": "1",  # Soles
            "tipo_de_cambio": 1.0,
            ...
            "venta_al_credito": [
                {
                    "cuota": 1,
                    "fecha_de_pago": (datetime.now() + timedelta(days=2)).strftime(
                        "%Y-%m-%d"
                    ),
                    "importe": 1000.00,
                }
    ...
    async def generate_pdf_with_qr_async(self, invoice_data, nubefact_response):
        ...
        try:
            ...
            invoice_data.update(
                {
                    ...
                    "cadena_para_codigo_qr": nubefact_response.get(
            ...
            template_name = invoice_data.get(
                "template", "billing/factura_electronica.html"
            )
    ...
    async def run_single_test(self, invoice_number=None):
        ...
        try:
            ...
            self.log(
                f"✅ Factura aceptada por SUNAT: {nubefact_response.get('aceptada_por_sunat', False)}"
            )
            ...
            if self.config["generate_pdf"]:
                ...
                pdf_content = await self.generate_pdf_with_qr_async(
                    invoice_data, nubefact_response
                )
            ...
            self.log(
                f"📄 Archivos generados: {len(self.test_results['generated_files'])}"
            )
    ...
    async def run_batch_test(self, count=3):
        ...
        self.log(
            f"📄 Total archivos: {sum(len(r.get('generated_files', [])) for r in results)}"
        )
...
def main():
    ...
    parser.add_argument(
        "--real", action="store_true", help="Usar NubeFact real (no simulado)"
    )
    ...
    if args.batch:
        ...
        if result["success"]:
            ...
            print(
                f"   Archivo: {result['generated_files'][0] if result['generated_files'] else 'No guardado'}"
            )

...
async def send_single_request(svc: NubefactServiceAsync, numero: int, semaphore: asyncio.Semaphore) -> Dict:
    ...
    async with NubefactServiceAsync() as svc:
        ...
        for i, result in enumerate(results):
            ...
            if status == 'SUCCESS':
                ...
                hash_short = result['response']['codigo_hash'][:20] if result['response'] else '???'
                print(f"{status:<10} {numero:<8} {duration:>6}ms        ✓ {hash_short}...")
            else:
                failed += 1
                error_short = (result['error'][:35] + '...') if len(result['error']) > 35 else result['error']
                print(f"{status:<10} {numero:<8} {duration:>6}ms        ✗ {error_short}")
        ...
        results_file = f"stress_results_async_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

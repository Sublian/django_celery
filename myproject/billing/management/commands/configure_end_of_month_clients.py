...
class Command(BaseCommand):
        ...
        with transaction.atomic():
            ...
            for company in companies:
                ...
                if not end_of_month_term:
                    self.stdout.write(f'   ⚠️  No se encontró término "Fin de Mes" para {company.partner.name}')
                    continue
                ...
                if enable_all:
                    updated = self._enable_all_partners(partners, end_of_month_term if set_term else None, dry_run)
    ...
    def _disable_all_partners(self, partners, dry_run=False):
        ...
        for partner in partners:
            ...
            count += 1
            if count <= 5:
                self.stdout.write(f'   ⚠️  {partner.name}: Pago fin de mes DESHABILITADO')
    ...
    def _configure_intelligent(self, partners, end_of_month_term, set_term=False, dry_run=False):
        ...
        for partner in partners:
            if partner.document_type == 'ruc' and partner.num_document and len(partner.num_document) == 11:
        ...
        for partner in corporate_clients:
            ...
            updated += 1
            if updated <= 3:
                status = "HABILITADO" if partner.invoice_end_of_month_payment else "deshabilitado"
                self.stdout.write(f'      • {partner.name}: Pago fin de mes {status}')
        
        # Regla: Personas naturales NO pagan a fin de mes (por defecto)
        self.stdout.write(f'   👤 Personas naturales: {len(individual_clients)} clientes')
        ...
        for partner in individual_clients:
            ...
            if updated - len(corporate_clients) <= 3:
                self.stdout.write(f'      • {partner.name}: Pago fin de mes DESHABILITADO (por defecto)')

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not Company.objects.exists():
            self.stdout.write(
                self.style.ERROR(
                    "❌ No hay compañías configuradas. Por favor, crea al menos una compañía antes de ejecutar este comando."
                )
            )
            return
    ...
    def populate_journals_and_sequences(self):
        ...
        for company in companies:
            ...
            if created:
                self.stdout.write(
                    f"    ✅ Diarios y secuencias creados para: {company} - {journal_factura.code}, {journal_boleta.code}"
                )
            else:
                self.stdout.write(
                    f"    ⚠️ Diarios y secuencias ya existe: {company} - {journal_factura.code}, {journal_boleta.code}"
                )
    ...
    def populate_invoice_series(self):
        ...
        for company in companies:
            ...
            journal_factura = Journal.objects.filter(
                company=company, code=f"INV{company.id}"
            ).first()

            journal_boleta = Journal.objects.filter(
                company=company, code=f"TKT{company.id}"
            ).first()

            sequence_factura = Sequence.objects.filter(
                company=company, code=f"sale.invoice.{company.id}"
            ).first()

            sequence_boleta = Sequence.objects.filter(
                company=company, code=f"sale.ticket.{company.id}"
            ).first()
            ...
            if journal_factura and sequence_factura:
                ...
                if created:
                    self.stdout.write(
                        f"    ✅ Serie Factura creada: {serie_factura.name}"
                    )
                else:
                    self.stdout.write(
                        f"    ⚠️ Serie Factura ya existe: {serie_factura.name}"
                    )
            ...
            if journal_boleta and sequence_boleta:
                ...
                if created:
                    self.stdout.write(
                        f"    ✅ Serie Boleta creada: {serie_boleta.name}"
                    )
                else:
                    self.stdout.write(
                        f"    ⚠️ Serie Boleta ya existe: {serie_boleta.name}"
                    )
    ...
    def populate_sunat_catalogs(self):
        ...
        catalogs = [
            ...
            {
                "code": "14",
                "name": "Gravado - Retiro por publicidad",
                "catalog_type": "10",
            },
            {"code": "15", "name": "Gravado - Bonificaciones", "catalog_type": "10"},
            {
                "code": "16",
                "name": "Gravado - Retiro por entrega a trabajadores",
                "catalog_type": "10",
            },
            ...
            {
                "code": "21",
                "name": "Exonerado - Transferencia Gratuita",
                "catalog_type": "10",
            },
            {
                "code": "30",
                "name": "Inafecto - Operación Onerosa",
                "catalog_type": "10",
            },
            {
                "code": "31",
                "name": "Inafecto - Retiro por Bonificación",
                "catalog_type": "10",
            },
            {"code": "32", "name": "Inafecto - Retiro", "catalog_type": "10"},
            {
                "code": "33",
                "name": "Inafecto - Retiro por Muestras Médicas",
                "catalog_type": "10",
            },
            {
                "code": "34",
                "name": "Inafecto - Retiro por Convenio Colectivo",
                "catalog_type": "10",
            },
            {
                "code": "35",
                "name": "Inafecto - Retiro por premio",
                "catalog_type": "10",
            },
            {
                "code": "36",
                "name": "Inafecto - Retiro por publicidad",
                "catalog_type": "10",
            },
            {
                "code": "37",
                "name": "Inafecto - Transferencia Gratuita",
                "catalog_type": "10",
            },
            ...
            {
                "code": "11",
                "name": "Ajustes de operaciones de exportación",
                "catalog_type": "note_type",
            },
            {
                "code": "12",
    ...
    def create_taxes(self):
        ...
        for company in companies:
            ...
            for tax_data in taxes_data:
                ...
                if created:
                    self.stdout.write(
                        f"    ✅ Impuesto creado: {tax.name} para {company}"
                    )
                else:
                    self.stdout.write(
                        f"    ⚠️ Impuesto ya existe: {tax.name} para {company}"
                    )
    ...
    def create_contract_templates(self):
        ...
        template_configs = [
            {
                "code": "15D",
                "name": "Contrato 15 Días",
                "interval": 15,
                "rule_type": "daily",
            },
            {
                "code": "1M",
                "name": "Plazo Forzoso 1 Mes",
                "interval": 1,
                "rule_type": "monthly",
            },
            {
                "code": "2M",
                "name": "Plazo Forzoso 2 Meses",
                "interval": 2,
                "rule_type": "monthly",
            },
            {
                "code": "3M",
                "name": "Plazo Forzoso 3 Meses",
                "interval": 3,
                "rule_type": "monthly",
            },
            {
                "code": "6M",
                "name": "Plazo Forzoso 6 Meses",
                "interval": 6,
                "rule_type": "monthly",
            },
            {
                "code": "1A",
                "name": "Plazo Forzoso 1 Año",
                "interval": 1,
                "rule_type": "yearly",
            },
        ]

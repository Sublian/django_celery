...
from billing.models import (
    Partner, ContractTemplate, SaleSubscription, AccountMove,
    AccountPaymentTerm, Company
)

class Command(BaseCommand):
    help = 'Establecer términos de pago por defecto en clientes, contratos y suscripciones'
    ...
    def handle(self, *args, **options):
        ...
        if not companies.exists():
            self.stdout.write(self.style.ERROR('❌ No se encontraron compañías para procesar.'))
            return  
        ...
        if dry_run:
            ...
            for company in companies:
                ...
                if not payment_terms.exists():
                    self.stdout.write(f'   ⚠️  No hay términos de pago para {company.partner.name}')
                    continue
    ...
    def _update_partners_payment_terms(self, company, default_term, dry_run=False):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'      ❌ Error actualizando partners: {str(e)}'))
            return 0
    ...
    def _update_subscriptions_payment_terms(self, company, dry_run=False):
        ...
        try:
            ...
            for subscription in subscriptions:
                ...
                if subscription.contract_template and hasattr(subscription.contract_template, 'payment_term'):
                    payment_term = subscription.contract_template.payment_term
                
                # Si no, intentar desde el partner
                if not payment_term and subscription.partner and hasattr(subscription.partner, 'payment_term'):
                ...
                if payment_term:
                    count += 1
                    if count <= 5:
                        self.stdout.write(f'      • Sub #{subscription.id}: {payment_term.name}')
                    elif count == 6:
                        self.stdout.write(f'      • ... y {subscriptions.count() - 5} más')
            
            return count
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'      ❌ Error actualizando suscripciones: {str(e)}'))
            return 0
        ...
        try:
            ...
            for invoice in invoices:
                ...
                if invoice.subscription:
                    ...
                    if not payment_term and invoice.subscription.contract_template and hasattr(invoice.subscription.contract_template, 'payment_term'):
                        payment_term = invoice.subscription.contract_template.payment_term
                    
                    # Si no, intentar desde el partner
                    if not payment_term and invoice.partner and hasattr(invoice.partner, 'payment_term'):
                    ...
                    if payment_term:
                        count += 1
                        if count <= 5:
                            self.stdout.write(f'      • Factura {invoice.invoice_number}: {payment_term.name}')
                        elif count == 6:
                            self.stdout.write(f'      • ... y {invoices.count() - 5} más')
            
            return count
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'      ❌ Error actualizando facturas: {str(e)}'))
            return 0
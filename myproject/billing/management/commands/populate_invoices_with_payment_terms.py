...
from billing.models import (
    AccountMove, SaleSubscription, Partner, Company,
...
class Command(BaseCommand):
    ...
    def _create_invoices_for_company(self, company, count, subscription_only=False):
        ...
        payment_terms = AccountPaymentTerm.objects.filter(company=company, is_active=True)
        
        if not all([partners.exists(), journals.exists(), series.exists(), products.exists()]):
        ...
        for i in range(min(count, partners.count())):
            try:
                ...
                subscription = None
                if subscriptions.exists() and (subscription_only or random.choice([True, False])):
                ...
                if subscription:
                    ...
                    payment_term = partner.payment_term or random.choice(list(payment_terms))
                ...
                # Mostrar información
                due_info = f"Vence: {invoice.invoice_date_due}" if invoice.invoice_date_due else "Sin vencimiento"
                self.stdout.write(f'   ✅ {reference}: {partner.name} - ${total_amount:.2f} - {payment_term.name if payment_term else "Sin término"} - {due_info}')

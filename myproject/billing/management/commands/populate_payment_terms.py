    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            for company in companies:
                ...
                stats = self._create_payment_terms_for_company(
                    company, force, dry_run
                )
    ...
    def _create_payment_terms_for_company(self, company, force=False, dry_run=False):
        ...
        for term_def in payment_terms_definitions:
            try:
                    ...
                    if not dry_run and term:
                        for line_def in term_def['lines']:
                            ...
                            if 'day_of_the_month' in line_def:
                                line_data['day_of_the_month'] = line_def['day_of_the_month']
                            if 'note' in line_def:
                ...
                self.stdout.write(
                    self.style.ERROR(f"   ❌ Error creando {term_def['name']}: {str(e)}")
                )

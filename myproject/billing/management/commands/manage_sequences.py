...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if company_id:
            try:
                ...
                self.stdout.write(f"📋 Procesando compañía: {company.partner.name} (ID: {company.id})")
            except Company.DoesNotExist:
                self.stdout.write(self.style.ERROR(f"❌ Compañía con ID {company_id} no encontrada"))
                return
        ...
        if options.get('sync'):
            ...
            self.reset_sequence(company_id, options['reset'], options.get('sequence_id'), verbose)
        else:
            ...
            try:
                ...
                if last_used is not None:
                    if sequence.number_next <= last_used:
                        ...
                        self.stdout.write(f"    ℹ️ {sequence.code}: Ya está sincronizada")
                elif verbose:
    ...
    def validate_sequences(self, company_id, verbose=False, fix=False):
        ...
        if inconsistencies:
            self.stdout.write(self.style.WARNING(f'⚠️  Se encontraron {len(inconsistencies)} inconsistencias:'))
            ...
            if fix:
                ...
                for inc in inconsistencies:
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(f"   ❌ Error corrigiendo {inc['sequence']}: {e}")
                        )
                
                self.stdout.write(self.style.SUCCESS(f"   ✅ Corregidas: {fixed_count}"))
            
            # Mostrar detalles
            for inc in inconsistencies:
                if 'error' in inc:
                    self.stdout.write(f"   ❌ {inc['sequence']}: ERROR - {inc['error']}")
                else:
                    self.stdout.write(
                        f"   ⚠️  {inc['sequence']}:\n"
        ...
        for seq in sequences.select_related('company'):
            try:
                ...
                if last_used is None:
                    ...
                    status = self.style.WARNING(f"⚠️  DESINCRONIZADA (+{last_used - seq.number_next})")
    ...
    def reset_sequence(self, company_id, new_number, sequence_id=None, verbose=False):
        ...
        if sequence_id:
            ...
            self.stdout.write(
                self.style.WARNING("⚠️  Se resetearán TODAS las secuencias. Use --sequence-id o --company para ser específico")
            )

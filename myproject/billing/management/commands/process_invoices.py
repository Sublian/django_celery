...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--post',
            action='store_true',
            help='Postear facturas en estado draft'
        )
        parser.add_argument(
            '--validate',
            action='store_true',
            help='Validar consistencia de totales'
        )
        parser.add_argument(
            '--fix',
            action='store_true',
    ...
    def _validate_invoices(self, service, verbose):
        ...
        self.stdout.write(f"📊 Resultados: {consistent_count} consistentes, {inconsistent_count} inconsistentes")
        ...
        for result in results:
            ...
            if not result['is_consistent'] and verbose:
                ...
                for line_detail in result['debug_info']:
                    ...
                    self.stdout.write(f"          TOTAL LÍNEA: ${line_detail['line_total']:.2f}")
    ...
    def _fix_invoices(self, service):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✅ {fixed_count} facturas corregidas')
        )

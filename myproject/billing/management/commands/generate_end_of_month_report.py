...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--month',
            type=int,
            help='Mes específico para el reporte (1-12)'
        )
        parser.add_argument(
            '--output',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        invoices = AccountMove.objects.filter(filters).select_related(
            'partner', 'company', 'invoice_payment_term'
        ).order_by('invoice_date')
        
        self.stdout.write(f'🔍 Encontradas {invoices.count()} facturas para el reporte')
        
        if not invoices.exists():
            self.stdout.write(self.style.WARNING('⚠️  No hay facturas para generar el reporte'))
            return
        ...
        for invoice in invoices:
            ...
            dias = (invoice.invoice_date_due - invoice.invoice_date).days if invoice.invoice_date_due else None
            
            # Información del cliente
            pago_fin_mes = "SÍ" if invoice.partner.invoice_end_of_month_payment else "NO"
            ...
            rows.append({
                ...
                'Fecha Vencimiento': invoice.invoice_date_due.strftime('%d/%m/%Y') if invoice.invoice_date_due else 'N/A',
                ...
                'Estado': invoice.get_state_display() if hasattr(invoice, 'get_state_display') else invoice.state
            })
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f'✅ Reporte generado exitosamente: {output_file}'))
            self.stdout.write(f'   📄 Filas escritas: {len(rows)}')
            self.stdout.write(f'   📅 Periodo: {report_year}' + (f'-{report_month:02d}' if report_month else ''))
    ...
    def _generate_statistics(self, invoices, year, month):
        ...
        con_hotfix = sum(1 for inv in invoices if self._determine_if_hotfix_applied(inv))
        con_pago_fin_mes = sum(1 for inv in invoices if inv.partner.invoice_end_of_month_payment)
        
        self.stdout.write(f'   • Total facturas: {total_facturas}')
        self.stdout.write(f'   • Con pago fin de mes: {con_pago_fin_mes} ({(con_pago_fin_mes/total_facturas*100):.1f}%)')
        self.stdout.write(f'   • Con hotfix aplicado: {con_hotfix} ({(con_hotfix/total_facturas*100):.1f}%)')
        ...
        if not month:
            ...
            for mes_num in sorted(meses_stats.keys()):
                ...
                porcentaje = (stats['hotfix'] / stats['total'] * 100) if stats['total'] > 0 else 0
                self.stdout.write(f'      • {nombre_mes}: {stats["total"]} facturas, {stats["hotfix"]} hotfix ({porcentaje:.1f}%)')

...
from billing.models import (
    Sequence, Company, Journal, InvoiceSerie, AccountMove
)
import logging
...
class Command(BaseCommand):
    help = 'Crear estructura completa de secuencias, diarios y series - VERSIÓN MEJORADA'
    ...
    def _fix_duplicate_sequences(self, companies, dry_run=False):
        ...
        for company in companies:
            ...
            for doc_type, seq_list in sequence_types.items():
                if len(seq_list) > 1:
                    self.stdout.write(f"      ⚠️  Duplicados encontrados para {doc_type}: {len(seq_list)} secuencias")
                    ...
                    if used_sequences:
                        ...
                        deactivate_ids = [seq.id for seq in seq_list if seq.id not in keep_ids]
                    else:
                    ...
                    if deactivate_ids:
                        if not dry_run:
                            Sequence.objects.filter(id__in=deactivate_ids).update(active=False)
                        
                        for seq_id in deactivate_ids:
                            seq = Sequence.objects.get(id=seq_id)
                            self.stdout.write(f"         ❌ Desactivando: {seq.code} (ID: {seq.id})")
    ...
    def _simulate_configuration(self, companies):
        ...
        for company in companies:
            ...
            for code in journal_codes:
                exists = Journal.objects.filter(code=f'{code}{company.id}', company=company).exists()
                status = "✅ Ya existe" if exists else "➕ Se crearía"
                self.stdout.write(f"    • {code}: {status}")
        ...
        for seq_data in essential_sequences:
            ...
            if created_flag:
                ...
                if updated_flag:
                    sequence.save()
                    updated += 1
                    self.stdout.write(f"      🔄 Secuencia actualizada: {sequence.name}")
        ...
        essential_journals = [
            {
                ...
                'type': 'sale',
                'sequence_code': f'sale.invoice.{company.id}' if invoice_sequence else None
            },
            {
                'code': f'TKT{company.id}',
                'name': f'Boletas de Venta - {company.partner.name}',
                'type': 'sale',
                'sequence_code': f'sale.ticket.{company.id}' if ticket_sequence else None
            },
            {
    ...
    def _ensure_essential_series(self, company):
        ...
        invoice_journal = Journal.objects.filter(
            code='INV1',
            company=company
        ).first()
        
        ticket_journal = Journal.objects.filter(
            code='TKT1',
            company=company
        ).first()
    ...
    def _show_summary(self, created, updated):
        ...
        self.stdout.write("\n🎯 RECOMENDACIONES:")
        self.stdout.write("   1. Verificar configuración: python manage.py verify_sequences")
        self.stdout.write("   2. Validar consistencia: python manage.py manage_sequences --validate")
        self.stdout.write("   3. Sincronizar secuencias: python manage.py manage_sequences --sync")

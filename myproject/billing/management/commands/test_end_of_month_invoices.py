...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--create-invoices", action="store_true", help="Crear facturas de prueba"
        )
        parser.add_argument(
            "--test-scenarios",
            action="store_true",
        ...
        parser.add_argument(
            "--year", type=int, default=2025, help="Año para las pruebas"
        )
    ...
    def _show_current_configuration(self, companies):
        ...
        for company in companies:
            ...
            end_of_month_clients = Partner.objects.filter(
                companies=company, invoice_end_of_month_payment=True, is_customer=True
            ).count()

            total_clients = Partner.objects.filter(
                companies=company, is_customer=True
            ).count()
            ...
            for client in sample_clients:
                term_info = (
                    f" - Término: {client.payment_term.name}"
            ...
            end_of_month_invoices = AccountMove.objects.filter(
                company=company, invoice_payment_term__lines__option="end_of_month"
            ).count()

            total_invoices = AccountMove.objects.filter(
                company=company, type="out_invoice"
            ).count()

            self.stdout.write(
                f"   🧾 Facturas con término fin de mes: {end_of_month_invoices}/{total_invoices}"
            )
    ...
    def _run_test_scenarios(self, companies, test_year):
        ...
        test_cases = [
            ...
            (
                ...
                True if calendar.isleap(test_year) else False,
            ),
        ...
        for company in companies:
            ...
            for i, (mes, dia, desc, espera_hotfix) in enumerate(test_cases, 1):
                try:
                    ...
                    invoice = AccountMove(
                        partner=test_client, company=company, invoice_date=test_date
                    )

                    # Simular que tiene término fin de mes
                    invoice.invoice_payment_term = AccountPaymentTerm.objects.filter(
                        company=company, lines__option="end_of_month"
                    ).first()
        ...
        with transaction.atomic():
            for company in companies:
                ...
                regular_term = AccountPaymentTerm.objects.filter(
                    company=company, lines__option="day_after_invoice_date"
                ).first()
                ...
                for client in clients_without_end_of_month:
                    try:
                        invoice = self._create_test_invoice(
                            client, company, service, journals, series, regular_term
                        )
    ...
    def _create_test_invoice(
        self, client, company, service, journals, series, payment_term
    ):
        """Crear una factura de prueba individual"""
        # Determinar tipo de documento
        if (
            client.document_type == "ruc"
            and client.num_document
            and len(client.num_document) == 11
        ):

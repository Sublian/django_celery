...
class InvoiceService:
    ...
    async def create_invoice(self, invoice_data, generate_pdf=True, save_to_disk=True, caller_info=None):
        ...
        try:
            ...
            # 1. Enviar a Nubefact
            print(f"📤 Enviando a NubeFact: {invoice_data.get('serie')}-{invoice_data.get('numero')}")
            ...
            nubefact_response = await nubefact_service.generar_comprobante(invoice_data_with_context)
            ...
            if generate_pdf:
                ...
                print(f"✅ PDF generado: {len(pdf_content) if pdf_content else 0} bytes")

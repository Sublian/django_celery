...
class Partner(TimeStampedModel):
    ...
    email_secondary = models.EmailField(
        blank=True, null=True, verbose_name="Correo Electrónico Secundario"
    )
    ...
    is_detractor = models.BooleanField(
        default=False, verbose_name="Sujeto a Detracción"
    )
    is_partner_retention = models.BooleanField(
        default=False, verbose_name="Sujeto a Retención"
    )
    street = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Dirección"
    )
    street2 = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Dirección 2"
    )
    phone = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="Teléfono"
    )
    mobile = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="Móvil"
    )
    ...
    sunat_last_check = models.DateTimeField(
        null=True, blank=True, verbose_name="Última verificación"
    )
    ...
    sunat_ubigeo = models.CharField(
        max_length=6, blank=True, verbose_name="Ubigeo SUNAT"
    )

    partner_latitude = models.DecimalField(
        max_digits=9, decimal_places=6, blank=True, null=True, verbose_name="Latitud"
    )
    partner_longitude = models.DecimalField(
    ...
    def update_sunat_status(
        self, data: dict = None, is_valid: bool = True, error: str = None
    ):
        """
        ...
        if not is_valid or error:
            ...
            self.sunat_comment = (
                f"[{self.sunat_last_check.date()}] ERROR: {error or 'RUC inválido'}"
            )
...
class Company(models.Model):
    partner = models.ForeignKey(
        Partner, on_delete=models.PROTECT, verbose_name="Partner Asociado"
    )
    sequence = models.CharField(max_length=64, verbose_name="Secuencia de Documentos")
    report_header = models.TextField(
        blank=True, null=True, verbose_name="Encabezado de Reportes"
    )
    report_footer = models.TextField(
        blank=True, null=True, verbose_name="Pie de Página de Reportes"
    )
    logo_web = models.ImageField(
        upload_to="company/logos/", blank=True, null=True, verbose_name="Logo para Web"
    )
    email = models.EmailField(blank=True, null=True, verbose_name="Correo Electrónico")
    ...
    invoice_term = models.TextField(
        blank=True, null=True, verbose_name="Términos y Condiciones de la Factura"
    )
    ...
    account_tax_periodicity_reminder_day = models.IntegerField(
        default=7, verbose_name="Día de Recordatorio de Periodicidad de Impuestos"
    )
    sunat_amount = models.DecimalField(
        max_digits=18, decimal_places=2, default=0, verbose_name="Monto para SUNAT"
    )
    currency_provider = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        default="sunat",
        verbose_name="Proveedor de Moneda Extranjera",
    )
    theme_color = models.CharField(
        max_length=20, default="light", verbose_name="Color del Tema"
    )
    theme_text_color = models.CharField(
        max_length=20, default="dark", verbose_name="Color del Texto del Tema"
    )
    text_color = models.CharField(
        max_length=20, default="black", verbose_name="Color del Texto"
    )
    company_color = models.CharField(
        max_length=20, default="blue", verbose_name="Color de la Compañía"
    )
    bank_comment = models.TextField(
        blank=True, null=True, verbose_name="Comentario Bancario"
    )
...
class Currency(models.Model):
    ...
    decimal_places = models.IntegerField(
        default=2, verbose_name="Decimales de Redondeo"
    )
    active = models.BooleanField(default=True, verbose_name="Activo")
    ...
    fraction_name = models.CharField(
        max_length=64, verbose_name="Nombre de la Fracción"
    )
...
class CurrencyRate(models.Model):
    currency = models.ForeignKey(
        Currency, on_delete=models.PROTECT, verbose_name="Moneda"
    )
    ...
    company = models.ForeignKey(
        Company, on_delete=models.PROTECT, verbose_name="Compañía"
    )
    purchase_rate = models.DecimalField(
        max_digits=18, decimal_places=6, verbose_name="Tasa de Compra", default=0
    )
    sale_rate = models.DecimalField(
        max_digits=18, decimal_places=6, verbose_name="Tasa de Venta", default=0
    )
...
class Product(models.Model):
    ...
    uom_code = models.CharField(
        max_length=5, default="NIU", choices=[("NIU", "Unidad"), ("ZZ", "Servicio")]
    )
...
class AccountPaymentTermLine(TimeStampedModel):
    ...
    payment_term = models.ForeignKey(
        AccountPaymentTerm, on_delete=models.CASCADE, related_name="lines"
    )
    sequence = models.IntegerField(default=10)
    days = models.IntegerField(
        default=0, help_text="Días después de la fecha de emisión"
    )
...
class SaleSubscription(TimeStampedModel):
    partner = models.ForeignKey(
        Partner, on_delete=models.PROTECT, help_text="Cliente asociado a la suscripción"
    )
    date_start = models.DateField(help_text="Fecha de inicio del contrato")
    date_end = models.DateField(
        null=True, blank=True, help_text="Fecha de finalización del contrato"
    )
    recurring_invoice_count = models.IntegerField(
        default=0, help_text="Número de facturas recurrentes generadas"
    )
    recurring_invoice_day = models.IntegerField(
        default=1, help_text="Día del mes para facturación recurrente"
    )
    recurring_total = models.DecimalField(
        max_digits=18,
        decimal_places=2,
        default=0,
        help_text="Total recurrente del contrato",
    )
    recurring_monthly = models.DecimalField(
        max_digits=18, decimal_places=2, default=0, help_text="Monto mensual recurrente"
    )
    company = models.ForeignKey(
        Company,
        on_delete=models.PROTECT,
        help_text="Compañía asociada a la suscripción",
    )
    state = models.CharField(
        max_length=20, default="active", help_text="Estado de la suscripción"
    )
    code = models.CharField(
        max_length=50,
        null=True,
        blank=True,
        help_text="Código interno de la suscripción",
    )
    description = models.TextField(
        blank=True, null=True, help_text="Descripción de la suscripción"
    )
    uuid = models.CharField(
        max_length=64,
        unique=True,
        help_text="Identificador único universal de la suscripción",
    )
    health = models.CharField(
        max_length=20, default="normal", help_text="Estado de salud de la suscripción"
    )
    to_renew = models.BooleanField(
        default=True,
        help_text="Indica si la suscripción debe renovarse automáticamente",
    )
    is_active = models.BooleanField(
        default=True, help_text="Indica si la suscripción está activa"
    )
    is_pu = models.BooleanField(
    ...
    next_invoice_date = models.DateField(
        null=True, blank=True, help_text="Próxima fecha programada para facturación"
    )
    invoicing_interval = models.CharField(
        max_length=20,
        choices=[
            ("daily", "Diario"),
            ("weekly", "Semanal"),
        ...
        default="monthly",
        help_text="Intervalo de facturación",
    )
    ...
    invoices_generated = models.IntegerField(
        default=0, verbose_name="Facturas Generadas"
    )
    total_invoiced = models.DecimalField(
        max_digits=18, decimal_places=2, default=0, verbose_name="Total Facturado"
    )
...
class SaleSubscriptionLine(TimeStampedModel):
    subscription = models.ForeignKey(
        SaleSubscription, related_name="lines", on_delete=models.CASCADE
    )
...
class AccountMove(TimeStampedModel):
    ...
    subscription = models.ForeignKey(
        SaleSubscription, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    narration = models.TextField(
        blank=True, null=True, help_text="Notas o Comentarios de la Factura"
    )
    invoice_date = models.DateField(
        default=timezone.now, help_text="Fecha de Emisión de la Factura"
    )
    invoice_date_due = models.DateField(
        blank=True, null=True, help_text="Fecha de Vencimiento de la Factura"
    )
    ...
    serie = models.ForeignKey(
        InvoiceSerie, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    detraction_type = models.ForeignKey(
        DetractionType, on_delete=models.SET_NULL, null=True, blank=True
    )
        ...
        if payment_line.option == "day_after_invoice_date":
            ...
            if payment_line.day_of_the_month:
                ...
                try:
                    ...
                    due_date = next_month.replace(
                        day=min(payment_line.day_of_the_month, last_day)
                    )
    ...
    def _calculate_end_of_month_with_hotfix(self):
        ...
        _, num_dias_en_mes = calendar.monthrange(
            self.invoice_date.year, self.invoice_date.month
        )
        ...
        condicion_b_dia_30_en_mes_31 = (
            self.invoice_date.day == 30 and num_dias_en_mes == 31
        )
        condicion_c_es_28_feb_en_bisiesto = (
            self.invoice_date.month == 2
            and self.invoice_date.day == 28
            and num_dias_en_mes == 29  # Año bisiesto
        )

        # Aplicar hotfix si se cumple alguna condición
        if (
            condicion_a_ultimo_dia
            or condicion_b_dia_30_en_mes_31
            or condicion_c_es_28_feb_en_bisiesto
        ):
    ...
    def _get_next_end_of_month(self, fecha_base):
        ...
        try:
            ...
            # Si el día no existe (ej: 30 en febrero), usar último día
            last_day = calendar.monthrange(
                primer_dia_mes_siguiente.year, mes_siguiente
            )[1]
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.invoice_payment_term
            and self.invoice_date
            and not self.invoice_date_due
        ):
...
class AccountMoveLine(TimeStampedModel):
    move = models.ForeignKey(
        AccountMove, related_name="lines", on_delete=models.CASCADE
    )
    product = models.ForeignKey(Product, on_delete=models.PROTECT)
    quantity = models.DecimalField(max_digits=10, decimal_places=2)
    price_unit = models.DecimalField(max_digits=15, decimal_places=2)
...
class ElectronicInvoice(TimeStampedModel):
    ...
    invoice = models.OneToOneField(
        AccountMove, on_delete=models.CASCADE, related_name="einvoice"
    )
    ...
    amount_total_origin = models.DecimalField(
...
class RelatedDocument(TimeStampedModel):
    ...
    invoice = models.ForeignKey(
        AccountMove, on_delete=models.CASCADE, related_name="related_documents"
    )
    document_type = models.ForeignKey(
        SunatCatalog, on_delete=models.PROTECT, limit_choices_to={"catalog_type": "01"}
    )
    series = models.CharField(max_length=4)
...
class GuideDocument(TimeStampedModel):
    ...
    invoice = models.ForeignKey(
        AccountMove, on_delete=models.CASCADE, related_name="guide_documents"
    )
...
class ProviderConfig(TimeStampedModel):
    ...
    billing_type = models.CharField(
        max_length=1, choices=[("0", "Nubefact"), ("1", "OdooFacturacion")]
    )
...
class SystemParameter(TimeStampedModel):
    ...
    advance_products = models.ManyToManyField(
        Product, blank=True, related_name="advance_parameters"
    )
...
class ContractTemplate(TimeStampedModel):
    ...
    user_closable = models.BooleanField(
        default=True, verbose_name="Usuario Puede Cerrar"
    )
    calculate_upsell = models.BooleanField(
        default=False, verbose_name="Calcular Upsell"
    )
...
class Sequence(TimeStampedModel):
    ...
    suffix = models.CharField(max_length=50, blank=True, verbose_name="Sufijo")
    number_next = models.IntegerField(
        default=1, validators=[MinValueValidator(1)], verbose_name="Próximo Número"
    )
    number_increment = models.IntegerField(
        default=1, validators=[MinValueValidator(1)], verbose_name="Incremento"
    )
    padding = models.IntegerField(
        default=8, validators=[MinValueValidator(1)], verbose_name="Relleno de ceros"
    )
    ...
    def get_next_number(self):
        ...
        with transaction.atomic():
            ...
            if next_number < 1:
                raise ValueError(
                    f"Número siguiente inválido ({next_number}) en la secuencia {self.code}"
                )
    ...
    def get_last_used_number(self):
        ...
        try:
            ...
            last_invoice = (
                AccountMove.objects.filter(serie=serie, invoice_number__isnull=False)
            ...
            logger.error(
                f"Error extrayendo número de referencia para secuencia {self.code}: {e}"
            )

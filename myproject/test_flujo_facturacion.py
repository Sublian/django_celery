...
def debug_cache_detallado():
    ...
    return {"test_passed": read_result == test_data, "apicache_passed": bool(read_tc)}
...
def mostrar_contenido_cache():
    ...
    ejemplos_ruc = ["ruc_20557425889", "ruc_20603274742", "ruc_20100035370"]
    ...
    for clave_ruc in ejemplos_ruc:
        ...
        if valor:
            ...
            if isinstance(valor, dict) and "razon_social" in valor:
                ...
                print(
                    f"   • Válido facturación: {valor.get('valido_facturacion', 'N/A')}"
                )
        else:
...
def obtener_tipo_cambio_dia(force_api=False):
    ...
    try:
        ...
        if moneda_usd:
            ...
            tipo_cambio_db = (
                CurrencyRate.objects.filter(currency=moneda_usd, date_rate=hoy)
                .order_by("-date_rate")
                .first()
            )
        ...
        if resultado.get("success"):
            ...
            try:
                moneda_usd, _ = Currency.objects.get_or_create(
                    name="USD",
                    defaults={"name": "US Dollar", "symbol": "$", "active": True},
                )
    ...
    try:
        ...
        if resultado_api.get("success"):
            ...
            # Normalizar campos
            compra = resultado_api.get("precio_compra") or resultado_api.get(
                "compra", 0
            )
    ...
    try:
        ...
        if moneda_usd:
            ultimo = (
                CurrencyRate.objects.filter(currency=moneda_usd)
                .order_by("-date_rate")
                .first()
            )
...
def obtener_rucs_para_validacion():
    ...
    print(
        f"   • Duplicados eliminados: {len(partners_rucs) + len(rucs_fallidos_unicos) - len(todos_rucs)}"
    )
...
def _consolidar_resultados_mejorado(
    resultados, total_esperado, total_procesado_api, rucs_omitidos=None
):
    """
    ...
    consolidated = {
        ...
        "validos_facturacion": {
            ...
            "porcentaje": (
                (len(validos) / total_resultados * 100) if total_resultados > 0 else 0
            ),
            "items": validos[:50],
        },
        "no_validos_facturacion": {
            ...
            "porcentaje": (
                (len(no_validos) / total_resultados * 100)
                if total_resultados > 0
                else 0
            ),
            "items": no_validos[:50],
        },
        "rucs_omitidos": {"cantidad": len(rucs_omitidos), "items": rucs_omitidos[:20]},
    }
...
def _consolidar_resultados(
    ...
    resultados_consolidados = {
        ...
        "rucs_omitidos": {
            ...
            "porcentaje": (
                (len(rucs_omitidos) / total_esperado * 100) if total_esperado > 0 else 0
            ),
        },
        "resumen_ejecucion": {
...
def validar_rucs_con_cache(ruc_list, max_lotes=2, max_reintentos=1):
    ...
    for ruc in ruc_list:
        ...
        if cache_data:
            cache_hits += 1
            resultados.append(
                {**cache_data, "cacheado": True, "fuente": "cache_individual"}
            )
        else:
                ...
                for ruc_data in resultado_lote.get("results", []):
                    if isinstance(ruc_data, dict):
                        ...
                        if ruc:
                            ...
                            es_valido = (
                                ruc_data.get("success", False)
                                and ruc_data.get("estado_del_contribuyente") == "ACTIVO"
                                and ruc_data.get("condicion_de_domicilio") == "HABIDO"
                            )

                            validacion = {
                                "ruc": ruc,
                                "razon_social": ruc_data.get(
    ...
    if rucs_omitidos and reintentos_realizados < max_reintentos:
        ...
        try:
            ...
            if resultado_reintento.get("success"):
                ...
                print(
                    f"    ✅ {rucs_procesados_en_reintento} RUCs procesados en reintento"
                )

                # Actualizar lista de omitidos
                rucs_recibidos = [
                    r.get("ruc")
                    for r in resultado_reintento.get("results", [])
                    if r.get("ruc")
                ]
                ...
                print(
                    f"    ⚠️  Reintento falló: {resultado_reintento.get('error', 'Error desconocido')}"
                )
                    ...
                    if resultado_individual.get("success"):
                        ...
                        for idx, resultado in enumerate(resultados):
                            if resultado.get("ruc") == ruc:
                                es_valido = (
                                    resultado_individual.get("estado_del_contribuyente")
                                    == "ACTIVO"
                                    and resultado_individual.get(
                                ...
                                resultados[idx] = {
                                    "ruc": ruc,
                                    "razon_social": resultado_individual.get(
                                        "nombre_o_razon_social", ""
                                    ),
                                    "estado": resultado_individual.get(
                                        "estado_del_contribuyente", ""
                                    ),
                                    "condicion": resultado_individual.get(
                                        "condicion_de_domicilio", ""
                                    ),
                                    "direccion": resultado_individual.get(
                                        "direccion", ""
                                    ),
                                    "actualizado_en": resultado_individual.get(
                                        "actualizado_en", ""
                                    ),
                                    "valido_facturacion": es_valido,
    ...
    resultados_consolidados = _consolidar_resultados_mejorado(
        resultados, len(ruc_list), rucs_procesados_api, rucs_omitidos
    )
...
def generar_reporte_facturacion(tipo_cambio, validacion_rucs):
    ...
    print(
        f"   ✅ Válidos para facturación: {validos['cantidad']} ({validos['porcentaje']:.1f}%)"
    )
    print(
        f"   ❌ No válidos: {no_validos['cantidad']} ({no_validos['porcentaje']:.1f}%)"
    )
...
def guardar_resultados_json(tipo_cambio, validacion_rucs, filename=None):
    ...
    datos_completos = {
        ...
        "resumen": {
            ...
            "clientes_no_validos": validacion_rucs["no_validos_facturacion"][
                "cantidad"
            ],
...
def flujo_completo_facturacion(max_lotes=2):
    ...
    try:
        ...
        validacion_rucs = validar_rucs_con_cache(
            rucs_data["todos"], max_lotes=max_lotes
        )
        ...
        return {"success": False, "error": str(e), "tiempo_total": elapsed_time}
...
def main():
    ...
    try:
        ...
        if opcion == "1":
            ...
            if (
                "rucs_omitidos" in validacion
                and validacion["rucs_omitidos"]["cantidad"] > 0
            ):
                print(
                    f"RUCs omitidos persistentes: {validacion['rucs_omitidos']['cantidad']}"
                )
            ...
            if json_files:
                ...
                generar_reporte_facturacion(
                    datos["tipo_cambio"], datos["validacion_rucs"]
                )
            else:
            ...
            print(f"\n✅ Debug completado")
            print(
                f"   Test básico: {'✅ PASÓ' if resultado['test_passed'] else '❌ FALLÓ'}"
            )
            print(
                f"   APICacheService: {'✅ PASÓ' if resultado['apicache_passed'] else '❌ FALLÓ'}"
            )

...
def consultar_rucs_masivo_task(rucs: list, batch_size=10, update_partners=False):
    ...
    result = async_to_sync(do_query)()
    logger.info(f"✅ Consulta masiva completada: {result['exitosos']}/{result['total']}")
    return result
...
def actualizar_partners_sunat():
    ...
    async def do_update():
        ...
        partners = Partner.objects.filter(
            last_sunat_check__isnull=True
        )[:1000]  # Limitar a 1000 por ejecución

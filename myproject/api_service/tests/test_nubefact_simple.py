...
def main():
    ...
    try:
        ...
        methods = ['generar_comprobante', 'consultar_comprobante', 'anular_comprobante', 
                   'send_request', '_get_endpoint', '_check_rate_limit']
        ...
        if log_count > 0:
            latest = ApiCallLog.objects.filter(service__service_type="NUBEFACT").order_by('-created_at').first()
            print("     Ultimo log:")
            print("       - Status: " + str(latest.status))
            print("       - Endpoint: " + (latest.endpoint.name if latest.endpoint else 'N/A'))

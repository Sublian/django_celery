...
def test_service_initialization():
    ...
    try:
        ...
        token_display = ("*" * 10) + (
            service.auth_token[-5:] if service.auth_token else "NO TOKEN"
        )
...
def test_generar_comprobante(service, log_count_before):
    ...
    try:
        ...
        print(
            "     - Aceptada por SUNAT: "
            + str(respuesta.get("aceptada_por_sunat", "N/A"))
        )
...
def test_logging(log_count_before):
    ...
    # Contar logs despues
    log_count_after = ApiCallLog.objects.filter(
...
def main():
    ...
    try:
        ...
        log_count_before = ApiCallLog.objects.filter(
            service__service_type="NUBEFACT"
        ).count()
        ...
        print(
            "   2. Verifica que ApiEndpoint este configurado para 'generar_comprobante'"
        )

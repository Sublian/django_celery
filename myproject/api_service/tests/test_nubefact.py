...
def test_nubefact_with_logging():
    ...
    try:
        ...
        new_logs = ApiCallLog.objects.filter(service__service_type="NUBEFACT").order_by('-created_at')
        ...
        if logs_count > initial_log_count:
            ...
            print(f"   Endpoint: {latest_log.endpoint.name if latest_log.endpoint else 'N/A'}")
            ...
            if latest_log.response_data:
                ...
                print(f"     - Estado SUNAT: {latest_log.response_data.get('aceptada_por_sunat', 'N/A')}")
        else:
            print("⚠️  No se encontraron nuevos logs. Verifica que el modelo ApiCallLog exista.")

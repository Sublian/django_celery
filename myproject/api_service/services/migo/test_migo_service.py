...
def test_migo_consultar_ruc_masivo_pequeño(migo_service):
    ...
    result = migo_service.consultar_ruc_masivo(
        rucs, batch_size=50, update_partners=False
    )
...
def test_migo_validar_rucs_masivo_facturacion(migo_service):
    ...
    try:
        ...
        if result.get("success"):
            ...
            print(
                f"    - Válidos para facturación: {result.get('validos_facturacion', 0)}"
            )
            print(
                f"    - Inválidos para facturación: {result.get('invalidos_facturacion', 0)}"
            )
            ...
            for val in result.get("validaciones", [])[:5]:  # Mostrar primeras 5
                print(
                    f"    - RUC {val.get('ruc')}: {'✅ VÁLIDO' if val.get('valido_facturacion') else '❌ INVÁLIDO'}"
                )
...
def test_migo_rate_limiting(migo_service):
    ...
    print(
        f"    - Tiempo de espera: {wait_time:.2f}s"
        if wait_time
        else "    - Tiempo de espera: 0s"
    )
...
def test_migo_complete_workflow(migo_service):
    ...
    if result_reps.get("success"):
        ...
        print(
            f"Cantidad de representantes: {len(data) if isinstance(data, list) else 'N/A'}"
        )

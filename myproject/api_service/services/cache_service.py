...
class APICacheService:
    ...
    def set_tipo_cambio(cls, fecha, data):
        ...
        cache_data = {
            **data,
            ...
            'expires_at': (datetime.now() + timedelta(seconds=cls.TTL_CONFIG['tipo_cambio'])).isoformat(),
    ...
    def set_ruc_lote(cls, ruc_list, data):
        ...
        cache_data = {
            ...
            'expires_at': (datetime.now() + timedelta(seconds=cls.TTL_CONFIG['ruc_lote'])).isoformat()
        }
    ...
    def clear_cache_type(cls, cache_type=None):
        ...
        if cache_type:
            ...
            print(f"⚠️  Para eliminar por tipo, necesita Redis u otro backend con pattern matching")
            return False
        else:
            cache.clear()
            return True

...
class APICacheService:
    ...
    def _verify_cache_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Conexión a cache exitosa (backend: {self.backend})")
            return True
    ...
    def clear_service_cache(self, service_name: str) -> Dict[str, int]:
        ...
        """
        cleaned_count = {"ruc_valid": 0, "ruc_invalid": 0, "tipo_cambio": 0, "total": 0}
    ...
    def set_tipo_cambio(
        self, fecha: str, data: Dict, ttl: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if result:
                logger.info(
                    f"Tipo cambio para {fecha} guardado en cache (ttl: {timeout}s)"
                )
            else:
    ...
    def add_invalid_ruc(
        self, ruc: str, reason: str = "INVALIDO", ttl_hours: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if result:
                logger.info(
                    f"RUC {ruc} agregado al cache de inválidos: {reason} (ttl: {ttl_seconds}s)"
                )
            else:
        ...
        try:
            ...
            if ruc in invalid_rucs:
                ...
                if invalid_rucs:
                    ...
                    oldest_entry = min(
                        ...
                        key=lambda x: datetime.fromisoformat(
                            x.get("added_at", datetime.now().isoformat())
                        ),
                    )
                ...
                if result:
                    ...
                    logger.warning(
                        f"No se pudo remover RUC {ruc} del cache de inválidos"
                    )
    ...
    def get_all_invalid_rucs(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            if len(valid_invalid_rucs) != len(invalid_rucs):
                self.set(
                    self.INVALID_RUCS_KEY, valid_invalid_rucs, self.RUC_INVALID_TTL
                )
        ...
        try:
            ...
            stats = {
                ...
                "invalid_rucs": {
                    ...
                    "breakdown_by_reason": self._breakdown_invalid_rucs_by_reason(
                        invalid_rucs
                    ),
                },
    ...
    def cleanup_expired(self) -> Dict[str, int]:
        ...
        """
        cleaned = {"invalid_rucs": 0, "valid_rucs": 0, "tipo_cambio": 0}
    ...
    def get_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health["status"] = (
                "healthy"
                if all_ok
                else "warning" if health["status"] != "unhealthy" else "unhealthy"
            )

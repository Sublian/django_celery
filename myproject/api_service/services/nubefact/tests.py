...
def nubefact_service(api_service_mock, api_endpoints_mock):
    ...
    with patch(
        "api_service.services.nubefact.base_service.ApiService.objects.filter"
    ) as mock_filter:
...
class TestEndpointLoading:
    ...
    def test_get_endpoint_retorna_endpoint_valido(
        self, mock_filter, nubefact_service, api_endpoints_mock
    ):
    ...
    @patch("api_service.services.nubefact.base_service.ApiEndpoint.objects.filter")
    def test_get_endpoint_retorna_none_si_no_existe(
        self, mock_filter, nubefact_service
    ):
    ...
    def test_send_request_falla_sin_endpoint(
        self, mock_get_endpoint, nubefact_service, sample_invoice_data
    ):
...
class TestRateLimiting:
            ...
            with patch.object(nubefact_service.session, "post") as mock_post:
                ...
                with patch.object(NubefactService, "_update_rate_limit"):
                    with patch.object(NubefactService, "_log_api_call"):
                        ...
                        result = nubefact_service.send_request(
                            "generar_comprobante", data
                        )
    ...
    def test_rate_limit_excedido(
        self, mock_log, mock_check, nubefact_service, sample_invoice_data
    ):
        ...
        with patch.object(NubefactService, "_get_endpoint") as mock_get_ep:
            ...
            with pytest.raises(NubefactAPIError, match="Rate limit"):
                nubefact_service.send_request(
                    "generar_comprobante", sample_invoice_data
                )
...
class TestOperationMethods:
    ...
    def test_generar_comprobante_llama_send_request(
        self, mock_send, nubefact_service, sample_invoice_data
    ):
    ...
    @patch.object(NubefactService, "send_request")
    def test_consultar_comprobante_prepara_datos_correctos(
        self, mock_send, nubefact_service
    ):
    ...
    @patch.object(NubefactService, "send_request")
    def test_anular_comprobante_prepara_datos_correctos(
        self, mock_send, nubefact_service
    ):
...
class TestValidation:
    ...
    def test_send_request_valida_datos(
        self, mock_validate, nubefact_service, sample_invoice_data
    ):
...
class TestErrorHandling:
    ...
    def test_handle_response_error_400(
        self, mock_check, mock_get_ep, nubefact_service, sample_invoice_data
    ):
        ...
        with patch.object(nubefact_service.session, "post") as mock_post:
            ...
            with patch.object(NubefactService, "_log_api_call"):
                with patch(
                    "api_service.services.nubefact.nubefact_service.validate_json_structure"
                ):
...
class TestLogging:
    ...
    def test_log_api_call_en_exito(
        self,
        mock_create,
        mock_check,
        mock_get_ep,
    ...
    @patch.object(NubefactService, "_check_rate_limit")
    @patch("api_service.services.nubefact.base_service.ApiCallLog.objects.create")
    def test_log_api_call_en_error(
        self,
        mock_create,
        mock_check,
        mock_get_ep,
...
class TestBatchRequest:
    ...
    @patch.object(NubefactService, "_check_rate_limit")
    @patch("api_service.services.nubefact.base_service.ApiCallLog.objects.create")
    def test_send_request_con_batch_request(
        self,
        mock_create,
        mock_check,
        mock_get_ep,
...
class TestIntegration:
    ...
    @patch("api_service.services.nubefact.base_service.ApiCallLog.objects.create")
    def test_flujo_completo_generar_comprobante(
        self,
        mock_create,
        mock_update,
        mock_check,

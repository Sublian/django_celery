...
from api_service.services.nubefact.exceptions import NubefactAPIError, NubefactValidationError
...
class NubefactServiceAsync(ABC):
    ...
    def _load_config_sync(self):
        ...
        try:
            ...
            if service:
                logger.info(f"Configuración cargada para {self.service_type}: {service.base_url}")
            return service
    ...
    async def _ensure_client(self) -> httpx.AsyncClient:
        if self._client is None or self._client.is_closed:
            self._client = httpx.AsyncClient(timeout=self.timeout, headers=self._build_headers())
        return self._client
    ...
    def _log_api_call_sync(self, endpoint_name: str, status_code: int, response_time_ms: int, error: str = None) -> None:
        """Sincrónico - se ejecuta en thread pool."""
        try:
            endpoint = self._get_endpoint_sync(endpoint_name)
            if endpoint and self.service:
                ...
                status = status_map.get(status_code, "FAILED" if status_code >= 400 else "SUCCESS")
    ...
    async def send_request(self, endpoint_name: str, data: Any, method: str = "POST", batch_request=None) -> dict:
        ...
        asyncio.create_task(self._log_api_call_async(endpoint_name, resp.status_code, duration_ms))
    ...
    def _handle_response_simple(self, response: httpx.Response) -> dict:
        ...
        if code == 400:
            error_msg = response_data.get('errors', 'Solicitud incorrecta') if isinstance(response_data, dict) else str(response_data)
    ...
    async def _update_rate_limit_async(self, endpoint_name: str) -> None:
        ...
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(self._executor, self._update_rate_limit_sync, endpoint_name)
        except Exception as e:
            logger.warning(f"Error updating rate limit async: {e}")

    async def _log_api_call_async(self, endpoint_name: str, status_code: int, response_time_ms: int, error: str = None) -> None:
        """Wrapper asincrónico para logging."""
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(self._executor, self._log_api_call_sync, endpoint_name, status_code, response_time_ms, error)
        except Exception as e:
            logger.warning(f"Error logging API call async: {e}")

    async def generar_comprobante(self, payload: dict, batch_request=None) -> dict:
        return await self.send_request("generar_comprobante", payload, method="POST", batch_request=batch_request)

    async def consultar_comprobante(self, numero: str, batch_request=None) -> dict:
        return await self.send_request("consultar_comprobante", {"numero": numero}, method="POST", batch_request=batch_request)

    async def anular_comprobante(self, numero: str, motivo: str, batch_request=None) -> dict:
        return await self.send_request("anular_comprobante", {"numero": numero, "motivo": motivo}, method="POST", batch_request=batch_request)

...
class InvoiceTestRunner:
    ...
    def send_to_nubefact_sync(self, invoice_data):
        ...
        try:
            ...
            if response.get("aceptada_por_sunat") is False:
                ...
                self.log(
                    f"    Descripción: {response.get('sunat_description')}", "warning"
                )
            else:
    ...
    def run_single_test(self, invoice_number=None):
        ...
        try:
            ...
            self.log(
                f"   Estado SUNAT: {'ACEPTADA' if nubefact_response.get('aceptada_por_sunat') else 'RECHAZADA'}"
            )
...
def main():
    ...
    if result["success"]:
        ...
        print(
            f"   Estado SUNAT: {'✅ ACEPTADA' if result['aceptada_por_sunat'] else '⚠️ RECHAZADA'}"
        )
        if result.get("generated_files"):
            print(f"   Archivo: {result['generated_files'][0]}")

        # Mostrar datos del QR
        if "nubefact_response" in result and result["nubefact_response"].get(
            "cadena_para_codigo_qr"
        ):
